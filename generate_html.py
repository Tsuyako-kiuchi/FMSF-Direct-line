...
def generate_html():
    # Excelファイル読み込み
    df = pd.read_excel("data.xlsx", sheet_name="電話帳リスト251113", engine="openpyxl")
    ...
    html = """<!DOCTYPE html><html lang='ja'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>電話帳</title>
    <style>
    ...
    .search-bar input { width: 60%; padding: 8px; font-size: 1em; margin-bottom: 5px; }
    .search-bar button { padding: 8px; margin-left: 5px; background: #555; color: white; border: none; cursor: pointer; font-size: 1em; }
    .category { margin: 10px; background: #fff; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
    .category h2 { margin: 0; padding: 10px; background: #eee; cursor: pointer; font-size: 1.2em; }
    ...
    @media (max-width: 600px) { .search-bar input { width: 100%; margin-bottom: 10px; } .search-bar button { width: 45%; margin-top: 5px; } }
    </style></head><body>
